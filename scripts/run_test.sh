...
help_text="
Script to run cpp driver matrix from within docker

    Optional values can be set via environment variables
    CPP_MATRIX_DIR, TOOLS_JAVA_DIR, JMX_DIR, CCM_DIR, SCYLLA_DBUILD_SO_DIR, CPP_DRIVER_DIR, INSTALL_DIRECTORY

    ./run_test.sh python3 main.py ../cpp-driver ../scylla
"
...
docker_cmd="docker run $(CPP_DRIVER_DOCKER_TAG) --detach=true \
    ${WORKSPACE_MNT} \
    ${DOCKER_COMMAND_PARAMS} \
    ...
    --network=bridge --privileged \
     bash -c 'yum install cmake libuv-devel openssl-devel krb5-devel;
          cd ${CPP_DRIVER_ORIG_DIR};pwd;ls -l;mkdir -p build && cd build && rm * && cmake -DCASS_BUILD_INTEGRATION_TESTS=ON .. && make;
